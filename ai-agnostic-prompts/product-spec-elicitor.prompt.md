# product-spec-elicitor · portable prompt

> **When to use:** Clarify a `new feature`, `Product modification`, `Product bug`, `cross-module change`, `新功能`, `修改需求`, `Bug 修复`, or `跨模块变更` after Product initialization.
>
> **Usage:** Paste everything below the next heading into an AI coding-agent conversation. Return the patch to `product-change-standardizer`; do not write Product files directly.
