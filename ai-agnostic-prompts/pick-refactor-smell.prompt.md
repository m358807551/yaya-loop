# Pick one refactor smell · portable prompt

> Use this workflow to scan Feature notes, rank unresolved code smells, recommend exactly one candidate, and hand the user's choice to Plan mode. English triggers: pick a smell to refactor, scan suggestions, find refactor TODOs. 中文触发：挑一个坏味道重构、扫一下 suggest、看看 notes 里的重构待办。
>
> Copy everything from `# Pick one refactor smell` onward into your AI coding agent. It is behaviorally identical to the native Claude Code Skill.
