# execute-next-feature · prompt

> **触发场景**：当用户要求实现下一个、当前、或指定的 feature 时使用。本 skill 从 docs/feature-list.json 中找到合适的 feature，按规范流程实现、验收并提交。触发短语示例："做下一个 feature"、"实现 F007"、"继续推进任务"、"开始干活"、"做下一步"。本 skill 不用于初始化 feature-list（用 generate-feature-list）、不用于源文档变更后的同步（用 sync-feature-list）。
>
> **用法**：把"# execute-next-feature"以下的全部内容粘到你 AI 对话窗口，AI 会按里面的步骤工作。
...
**理由**：主上下文走完阶段 0-5 后已被压缩，编码规则细节容易遗忘；fresh context 能完整加载规则做精确扫描。

**怎么委派**（按你 AI CLI 的能力选）：
- **支持 sub-agent / 多窗口的 CLI**（如 Aider 多 session / Cursor Composer 新会话）：直接开一个新会话/新窗口，粘下面的 prompt 模板，把返回 JSON 贴回当前会话。
- **不支持的 CLI**：提示用户「请打开一个新的 AI 会话/窗口，粘贴下面的 prompt 模板，把扫描结果以 JSON 形式贴回当前会话」，等用户回贴后继续 6.2。

**子 agent / 新会话使用的 prompt 模板**：
...
**最后一行是阶段 7 的准入证据，必须替换成当前 feature id 并原样输出。`must_fix` 表示扫描和修复后剩余的阻断项，所以进入阶段 7 时必须为 `0`。**
