# Execute one Feature · portable prompt

> Use this workflow to execute the next, current, or specified Feature through implementation, verification, human acceptance, independent review, completion, and handoff. English triggers: do the next Feature, implement F007, continue the task. 中文触发：做下一个 feature、实现 F007、继续推进任务、开始干活。
>
> Copy everything from `# Execute one Feature` onward into your AI coding agent. It is behaviorally identical to the native Claude Code Skill.
