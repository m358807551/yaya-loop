# product-init-elicitor · portable prompt

> **When to use:** `Initialize the Product`, `start a project from one sentence`, `初始化产品`, or `做一个 XXX`, when `docs/product.md` is absent or still an empty template.
>
> **Usage:** Paste everything below the next heading into an AI coding-agent conversation. The agent may use any equivalent interactive input mechanism.
