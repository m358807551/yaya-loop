# generate-feature-list · prompt

> **触发场景**：当用户要求首次生成、初始化、或彻底重新生成 docs/feature-list.json 时使用。本 skill 读取 docs/product.md（总览）、docs/product/ 目录下所有模块文件、以及 docs/coding_rules.md，按依赖顺序拆解出可逐步实现的 feature 列表。仅用于"从零到一"或"推倒重来"。增量更新请用 sync-feature-list。触发短语示例："生成 feature-list"、"初始化任务列表"、"拆解游戏功能"。
>
> **用法**：把"# generate-feature-list"以下的全部内容粘到你 AI 对话窗口，AI 会按里面的步骤工作。
