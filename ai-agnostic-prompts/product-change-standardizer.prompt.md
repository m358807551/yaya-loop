# product-change-standardizer · prompt

> **触发场景**：产品变更总调度。任何涉及产品需求、功能、UI、音效、Bug 修复的变更都从这里进入。负责路由到合适的子 skill、统一写入模块文件、调用同步 skill。这是产品变更的唯一入口。
>
> **用法**：把"# product-change-standardizer"以下的全部内容粘到你 AI 对话窗口，AI 会按里面的步骤工作。
