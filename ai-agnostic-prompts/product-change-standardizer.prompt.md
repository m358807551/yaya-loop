# product-change-standardizer · portable prompt

> **When to use:** Product initialization, a new feature, Product improvement, behavior correction, UI/audio requirement, or Product-level bug. Trigger examples include `initialize`, `improve`, `add`, `remove`, `fix`, `做一个 XXX`, `改进`, `增加`, `删除`, and `修复`.
>
> **Usage:** Paste everything below the next heading into an AI coding-agent conversation. Named child Skills may be invoked through native capabilities or executed from their corresponding portable Prompts.
